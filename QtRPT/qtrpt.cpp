...
void QtRPT::setTableMap(const QVariantList &vlist, const QVariant &mappo)
{
    m_vlist = vlist;
    m_mapOne = mappo.toMap();
}

void QtRPT::setActivedSignal(const bool &bm)
{
    ...
    for (int i = 0; i < res.size(); ++i) {
        if (res.at(i).contains("[") && res.at(i).contains("]") && !res.at(i).contains("<")) {
            ...
            if (res[i].contains("<") && res[i].contains(">")) {
                ...
                QJSValue fun = myEngine.newQObject(new MyExtension());
                myEngine.globalObject().setProperty("Sum", fun.property("funcAggregate"));

                myEngine.globalObject().setProperty("NumberToWords", fun.property("funcText"));

                myEngine.globalObject().setProperty("Frac", fun.property("funcFrac"));

                myEngine.globalObject().setProperty("Floor", fun.property("funcFloor"));

                myEngine.globalObject().setProperty("Ceil", fun.property("funcCeil"));

                myEngine.globalObject().setProperty("Round", fun.property("funcRound"));
...
//    return paramValue.toString();
    if(m_boolActiveSignal){
        //qDebug()<<m_recNo<<paramName<<m_pageReport;
...
void QtRPT::printExec(bool maximum, bool direct, QString printerName)
{
    ...
    if (!direct) {
        ...
        connect(actExpToHtml, &QAction::triggered, this, &QtRPT::exportTo);
            ...
            connect(actExpToXlsx, &QAction::triggered, this, &QtRPT::exportTo);
        #endif
...
QJSValue MyExtension::funcAggregate(const QJSValue &arg1, const QJSValue &arg2)
{
    QJSEngine myEngine;
    bool boolSG=myEngine.evaluate("showInGroup").toBool();
    //qDebug()<<value<<"agre"<<value2<<"x"<<vz;

    int funcMode = arg1.toNumber();
    QString paramName = arg2.toString();
    double total = 0;
    double min = 0;
    double max = 0;
    int count = 0;
    ...
    for (int i=0; i<listOfPair.size(); i++) {
        ...
        if (listOfPair.at(i).paramName == paramName) {
            if (listIdxOfGroup.size() > 0 && boolSG == true) {
...
QJSValue MyExtension::funcText(const QJSValue &arg1, const QJSValue &arg2)
{
    //qDebug()<<value<<"valueri";
    QString paramLanguage = arg1.toString();
    double value = arg2.toNumber();
    return double2Money(value,paramLanguage);
}

QJSValue MyExtension::funcFrac(const QJSValue &arg1)
{
    double value = arg1.toNumber();
    int b = qFloor(value);
    b = (value-b) * 100+0.5;
    return b;
}

QJSValue MyExtension::funcFloor(const QJSValue &arg1)
{
    double value = arg1.toNumber();
    return qFloor(value);
}

QJSValue MyExtension::funcCeil(const QJSValue &arg1)
{
    double value = arg1.toNumber();
    return qCeil(value);
}

QJSValue MyExtension::funcRound(const QJSValue &arg1)
{
    double value = arg1.toNumber();
    return qRound(value);
}
